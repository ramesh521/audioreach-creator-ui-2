...
Scripts live at `.ai/skills/brainstorming/scripts/` (the tracked source —
resolves correctly from the repo root even before `setup-ai-links.js` has
run). Run commands from the repo root.

```bash
# Start AFTER the user approves the companion. --open auto-opens their browser on
# the first screen; --project-dir persists mockups and enables same-port restart.
.ai/skills/brainstorming/scripts/start-server.sh --project-dir /path/to/project --open
...
**Note:** Pass the project root as `--project-dir` so mockups persist in `.brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up.
...
.ai/skills/brainstorming/scripts/start-server.sh --project-dir /path/to/project --open --foreground
```
...
```bash
.ai/skills/brainstorming/scripts/start-server.sh \
...
```bash
.ai/skills/brainstorming/scripts/stop-server.sh $SESSION_DIR
```
