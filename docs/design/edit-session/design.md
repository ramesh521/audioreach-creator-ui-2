...
- **New response-reconciliation contract.** Confirmed backend endpoints
  ...
  create, an `{added, updated, removed}` triple on subsystem move-in/
  move-out, a deleted-entities collection on module delete) and which
  narrow single-entity/
...
`POST /subsystems/{id}/components/move-out`, then
...
behavior — Expand's own move-out-then-delete sequence is exactly how a
non-empty subsystem is deleted; there is no separate cascading-delete path.
...
- **Whether `move-in` rejects a descendant-nesting cycle.** The
...
- **Whether a port-count decrease via `PATCH /spf-modules/{id}`/
  ...
  response (the full updated `SpfModuleDto`/`SubsystemDto`) does not itself
