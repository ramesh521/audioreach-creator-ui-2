...
jest.mock('~features/subsystem-browser/ui/search-box', () => ({
  __esModule: true,
  default: ({
    onChange,
    searchTerm,
...
jest.mock('~features/subsystem-browser/ui/subsystem-tree-node', () => ({
  __esModule: true,
  default: ({
    isExpanded,
    onClick,
...
jest.mock(
  '~features/subsystem-browser/model/use-subsystem-browser-store',
  () => ({
...
describe('SubsystemTreeView', () => {
  ...
  it('remove subsystem removes a nested node and its subtree', () => {
    ...
    expect(root?.children?.some((c: {id: number}) => c.id === 1231)).toBe(
      false,
    );
  });
