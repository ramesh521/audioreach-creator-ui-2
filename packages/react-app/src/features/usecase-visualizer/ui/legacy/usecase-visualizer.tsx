...
// UsecaseVisualizerLegacy - ReactFlow wrapper (view-only)
...
export interface UsecaseVisualizerLegacyProps {
  edges: RFEdge[];
  nodes: RFNode[];
...
const FlowContent: FC<UsecaseVisualizerLegacyProps> = ({
  edges,
  nodes,
  onScreenshotReady,
  projectId,
  ...
  useEffect(() => {
    return () => {
      logger.verbose(
        `UsecaseVisualizerLegacy unmounting, clearing selections for project: ${projectId}`,
      );
      clearSelection(projectId);
      clearKeyConfiguratorSelection();
    };
...
export const UsecaseVisualizerLegacy: FC<UsecaseVisualizerLegacyProps> = ({
  edges,
  nodes,
  onScreenshotReady,
  projectId,
