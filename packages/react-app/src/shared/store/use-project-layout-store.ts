...
export const useProjectLayoutStore = create<ProjectLayoutStore>((set, get) => ({
  ...
  addPanelTab: (
    ...
    if (updated) {
      ...
      set((state) => {
        ...
        const newPanelTabRegistry = new Map(state.panelTabRegistry || new Map())
  ...
  getLayoutConfig: (projectGroupId: string): string | null => {
    ...
    return layout?.flexLayoutData ? JSON.stringify(layout.flexLayoutData) : null
  },
      ...
      if (state.activeTabGroup?.id === appGroupId) {
        if (updatedAppGroups.length > 0) {
          ...
          if (firstProject.projectTabs.length > 0) {
            ...
            newActiveTab =
              firstProject.projectTabs.find((t) => t.id === targetTabId) || null
